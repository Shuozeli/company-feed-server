-- The production company-scope audit found three additional broad publication
-- roots whose structurally valid articles belong to many unrelated issuers.
-- Preserve their raw evidence, stale the recipes, and reversibly quarantine
-- every public item attributed through those exact company/publication pairs.

CREATE TEMP TABLE additional_company_scope_repairs ON COMMIT DROP AS
...
CREATE TEMP TABLE additional_company_scope_items ON COMMIT DROP AS
SELECT
    ...
    item.title,
    repair.reason
FROM additional_company_scope_repairs AS repair
JOIN feed_items AS item ON item.source_id = repair.source_id
WHERE NOT item.is_private;

CREATE TEMP TABLE additional_company_scope_wave (
...
WITH repair_started AS (
    ...
    SELECT
        ...
        jsonb_build_object(
            'recipe_count', (SELECT count(*) FROM additional_company_scope_repairs),
            'item_count', (SELECT count(*) FROM additional_company_scope_items),
            'policy', 'company-scope-relevance.v1',
            'migration', '0029_quarantine_additional_unscoped_publications'
        )
    WHERE EXISTS (SELECT 1 FROM additional_company_scope_repairs)
    RETURNING id
)
INSERT INTO additional_company_scope_wave (event_id)
...
FROM
    additional_company_scope_repairs AS repair
    CROSS JOIN additional_company_scope_wave AS wave
...
FROM
    additional_company_scope_items AS repair
    CROSS JOIN additional_company_scope_wave AS wave
...
FROM additional_company_scope_items AS repair
