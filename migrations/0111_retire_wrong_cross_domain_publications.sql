-- Retire three confirmed cross-domain ownership mistakes. CrewAI is a
-- separate agent-platform company from Bolto (whose historical launch name
-- was "Crew AI"), and Paytient is separate from Mark Cuban Cost Plus Drug
-- Company. The generic runtime now keeps future unrelated adapter domains
-- company-identity scoped.

CREATE TEMP TABLE wrong_cross_domain_sources
ON COMMIT DROP AS
...
CREATE TEMP TABLE wrong_cross_domain_recipes
ON COMMIT DROP AS
SELECT
...
CREATE TEMP TABLE wrong_cross_domain_wave (
...
WITH repair_started AS (
    ...
    SELECT
        ...
        jsonb_build_object(
            'source_count',
                (SELECT count(*) FROM wrong_cross_domain_sources),
            'recipe_count',
                (SELECT count(*) FROM wrong_cross_domain_recipes),
            'item_count',
                (SELECT count(*) FROM wrong_cross_domain_items),
            'company_count',
                (
                    SELECT count(DISTINCT company_id)
                    FROM wrong_cross_domain_sources
                ),
            'policy', 'cross-domain-company-ownership.v1',
            'migration', '0111_retire_wrong_cross_domain_publications'
        )
    WHERE EXISTS (SELECT 1 FROM wrong_cross_domain_sources)
    RETURNING id
)
INSERT INTO wrong_cross_domain_wave (event_id)
...
FROM wrong_cross_domain_recipes AS repair
...
FROM
    wrong_cross_domain_recipes AS repair
    CROSS JOIN wrong_cross_domain_wave AS wave
...
SET
    ...
    lease_until = NULL,
    lease_token = NULL
FROM wrong_cross_domain_sources AS repair
WHERE
...
FROM
    wrong_cross_domain_items AS repair
    CROSS JOIN wrong_cross_domain_wave AS wave
...
FROM wrong_cross_domain_items AS repair
