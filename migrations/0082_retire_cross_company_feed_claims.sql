-- A recipe must not claim a publication whose complete article set is already
-- owned by an approved RSS/Atom source for a distinct issuer. These three
-- associations predate the generic cross-company feed-evidence guard.

CREATE TEMP TABLE wrong_cross_company_feed_recipes
ON COMMIT DROP AS
WITH rejected_source (
    ...
    VALUES
        (
            ...
            'https://www.arkocorp.com/news-events/press-releases',
            'ARKO Corp. Common Stock'
        ),
        (
            'blue-owl-capital-inc-class-a-common-stock',
            'https://www.blueowlcapitalcorporation.com/investors/news-events/press-releases',
            'Blue Owl Capital Corporation Common Stock'
        ),
        (
            'cheniere-energy-partners-lp-common-units',
            'https://lngir.cheniere.com/news-events/press-releases',
...
WHERE
    ...
    AND (
        public_url_identity_key(recipe.spec->>'publication_url')
            = public_url_identity_key(rejected_source.publication_url)
        OR public_url_identity_key(source.url)
            = public_url_identity_key(rejected_source.publication_url)
    );

CREATE TEMP TABLE wrong_cross_company_feed_items
ON COMMIT DROP AS
SELECT
    ...
    item.published_at,
    repair.reason
FROM wrong_cross_company_feed_recipes AS repair
JOIN feed_items AS item ON item.source_id = repair.source_id
WHERE NOT item.is_private;

CREATE TEMP TABLE wrong_cross_company_feed_wave (
...
WITH repair_started AS (
    ...
    SELECT
        ...
        jsonb_build_object(
            'recipe_count',
                (SELECT count(*) FROM wrong_cross_company_feed_recipes),
            'item_count',
                (SELECT count(*) FROM wrong_cross_company_feed_items),
            'company_count',
                (
                    SELECT count(DISTINCT company_id)
                    FROM wrong_cross_company_feed_recipes
                ),
            'policy', 'cross-company-feed-ownership.v1',
            'migration', '0082_retire_cross_company_feed_claims'
        )
    WHERE EXISTS (SELECT 1 FROM wrong_cross_company_feed_recipes)
    RETURNING id
)
INSERT INTO wrong_cross_company_feed_wave (event_id)
...
FROM
    wrong_cross_company_feed_recipes AS repair
    CROSS JOIN wrong_cross_company_feed_wave AS wave
...
FROM
    wrong_cross_company_feed_items AS repair
    CROSS JOIN wrong_cross_company_feed_wave AS wave
...
FROM wrong_cross_company_feed_items AS repair
