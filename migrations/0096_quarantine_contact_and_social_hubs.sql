-- Media-contact directories and corporate social-channel indexes are newsroom
-- navigation, not articles. The shared content policy now rejects these exact
-- terminal resources. Quarantine historical matches and retire recipes whose
-- accepted output consisted only of those utility pages.

CREATE TEMP TABLE newsroom_utility_items
ON COMMIT DROP AS
...
CREATE TEMP TABLE newsroom_utility_wave (
...
WITH repair_started AS (
    ...
    SELECT
        'company_news.newsroom_utility_backfill_started',
        jsonb_build_object(
            'item_count',
                (SELECT count(*) FROM newsroom_utility_items),
            'company_count',
                (
                    SELECT count(DISTINCT company_id)
                    FROM newsroom_utility_items
                ),
            'source_count',
                (
            ...
            'migration',
                '0096_quarantine_contact_and_social_hubs'
        )
    WHERE EXISTS (
        SELECT 1 FROM newsroom_utility_items
    )
    RETURNING id
)
INSERT INTO newsroom_utility_wave (event_id)
...
SET
    ...
    content_processing = item.content_processing || jsonb_build_object(
        ...
        jsonb_build_object(
            'state', 'quarantined',
            'reason', 'non_editorial_newsroom_utility',
...
FROM
    newsroom_utility_items AS repair
    CROSS JOIN newsroom_utility_wave AS wave
WHERE
...
SET
    ...
    normalization_error =
        'quality quarantine: non_editorial_newsroom_utility',
    normalized_feed_item_id = NULL
FROM newsroom_utility_items AS repair
...
CREATE TEMP TABLE newsroom_utility_only_recipes
ON COMMIT DROP AS
SELECT DISTINCT
    recipe.id AS recipe_id,
    recipe.company_id,
...
FROM
    ...
    JOIN newsroom_utility_items AS repair
...
SET
    ...
    stale_at = NULL,
    stale_reason = NULL
FROM newsroom_utility_only_recipes AS invalid
WHERE
...
SET
    ...
    reason = 'non_editorial_newsroom_utility_only_recipe',
    metadata = COALESCE(state.metadata, '{}'::jsonb) || jsonb_build_object(
        'supersession',
        jsonb_build_object(
            'policy', 'company-news-utility-scope.v2',
            'reason', 'non_editorial_newsroom_utility_only_recipe',
...
SET
    ...
    last_error = 'non-editorial newsroom utility-only recipe superseded'
FROM newsroom_utility_only_recipes AS invalid
WHERE
