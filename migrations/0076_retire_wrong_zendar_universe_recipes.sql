-- Zendar (YC W17, zendar.io) builds automotive radar systems. Zendar
-- Universe (zendaruniverse.com) is an independent astronomy research
-- nonprofit. Historical discovery confused the shared name and activated
-- Zendar Universe publications for Zendar. Retire those recipes and
-- quarantine their items without changing Zendar's valid legal-name alias.

CREATE TEMP TABLE wrong_zendar_universe_recipes
ON COMMIT DROP AS
SELECT
...
WHERE
    company.company_key = 'yc-zendar'
    AND recipe.status = 'active'
    AND (
        lower(recipe.spec->>'publication_url') ~
            '^https?://([^/]+\.)?zendaruniverse\.com(/|$)'
        OR lower(source.url) ~
            '^https?://([^/]+\.)?zendaruniverse\.com(/|$)'
    );

CREATE TEMP TABLE wrong_zendar_universe_items
ON COMMIT DROP AS
SELECT
    ...
    repair.reason
FROM
    wrong_zendar_universe_recipes AS repair
    JOIN feed_items AS item ON item.source_id = repair.source_id
WHERE NOT item.is_private;

CREATE TEMP TABLE wrong_zendar_universe_wave (
    ...
    SELECT
        ...
        jsonb_build_object(
            'recipe_count',
                (
                    SELECT count(*)
                    FROM wrong_zendar_universe_recipes
                ),
            'item_count',
                (
                    SELECT count(*)
                    FROM wrong_zendar_universe_items
                ),
            'company_count',
                (
            ...
            'target_company_key', 'yc-zendar',
            'rejected_publication_host', 'zendaruniverse.com',
            'policy', 'company-ownership.v1',
            'migration',
                '0076_retire_wrong_zendar_universe_recipes'
        )
    WHERE EXISTS (
        SELECT 1 FROM wrong_zendar_universe_recipes
    )
    RETURNING id
)
INSERT INTO wrong_zendar_universe_wave (event_id)
...
FROM wrong_zendar_universe_recipes AS repair
...
FROM
    wrong_zendar_universe_recipes AS repair
    CROSS JOIN wrong_zendar_universe_wave AS wave
...
FROM
    wrong_zendar_universe_items AS repair
    CROSS JOIN wrong_zendar_universe_wave AS wave
...
FROM wrong_zendar_universe_items AS repair
