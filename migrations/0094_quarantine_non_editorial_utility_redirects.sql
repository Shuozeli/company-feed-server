-- Editorial listings sometimes link through a company-owned URL that redirects
-- to a Help Center, warranty page, FAQ, or product-support document. Older
-- recipe crawls accepted the fetched page after proving only the original link
-- boundary. The crawler now evaluates final and canonical URLs with the shared
-- utility-content policy. Backfill that deterministic rule, while preserving
-- explicit blogs and release-note collections hosted under documentation or
-- support paths.
...
classified AS (
    SELECT
        ...
        (
            regexp_replace(
                split_part(item.item_url, '://', 2),
                '/.*$',
                ''
            ) ~ '^(help|helpcenter|knowledgebase|support)\.'
            OR regexp_replace(
                split_part(item.canonical_item_url, '://', 2),
                '/.*$',
                ''
            ) ~ '^(help|helpcenter|knowledgebase|support)\.'
            OR item.item_url ~
                '/(docs|documentation|faq|faqs|hc|help|knowledge-base|knowledgebase|support)(/|$)'
        ...
        (
            ...
            OR item.canonical_item_url ~
                '/(blog|blogs|changelog|changelogs|company-news|engineering|insights|news|newsroom|press|press-release|press-releases|pressrelease|pressreleases|product-updates|release-notes|research|stories|updates|what-s-new|whats-new)(/|$)'
            OR item.item_url ~ '(release-notes|product-updates)'
            OR item.canonical_item_url ~ '(release-notes|product-updates)'
            OR item.item_url ~
                'releases-for-[^/]*(19|20)[0-9]{2}'
            OR item.canonical_item_url ~
                'releases-for-[^/]*(19|20)[0-9]{2}'
...
CREATE TEMP TABLE utility_quarantine_wave (
...
WITH repair_started AS (
    ...
    SELECT
        'company_news.utility_redirect_backfill_started',
        jsonb_build_object(
            'item_count',
            ...
            'migration',
                '0094_quarantine_non_editorial_utility_redirects'
        )
    WHERE EXISTS (
        SELECT 1 FROM non_editorial_utility_items
    )
    RETURNING id
)
INSERT INTO utility_quarantine_wave (event_id)
...
SET
    ...
    content_processing = item.content_processing || jsonb_build_object(
        ...
        jsonb_build_object(
            'state', 'quarantined',
            'reason', 'non_editorial_utility_scope',
...
FROM
    ...
    CROSS JOIN utility_quarantine_wave AS wave
WHERE
...
SET
    ...
    normalization_error =
        'quality quarantine: non_editorial_utility_scope',
...
CREATE TEMP TABLE utility_only_recipes
ON COMMIT DROP AS
SELECT DISTINCT
    recipe.id AS recipe_id,
    recipe.company_id,
...
FROM
    ...
    JOIN non_editorial_utility_items AS repair
...
SET
    ...
    stale_at = NULL,
    stale_reason = NULL
FROM utility_only_recipes AS invalid
WHERE
...
SET
    ...
    reason = 'non_editorial_utility_only_recipe',
    metadata = COALESCE(state.metadata, '{}'::jsonb) || jsonb_build_object(
        'supersession',
        jsonb_build_object(
            'policy', 'company-news-utility-scope.v1',
            'reason', 'non_editorial_utility_only_recipe',
...
SET
    ...
    last_error = 'non-editorial utility-only recipe superseded'
FROM utility_only_recipes AS invalid
WHERE
